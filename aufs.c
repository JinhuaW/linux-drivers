	...
	*dentry = lookup_one_len(name, parent, strlen(name));
	if (!IS_ERR(dentry)) {
		if ((mode & S_IFMT) == S_IFDIR)
			...
			error = aufs_create(parent->d_indoe, *dentry, mode);
	} else {
...
struct dentry *aufs_create_file(const char *name, mode_t mode, struct dentry *parent, void *data, struct file_operations *fops)
{
	...
	if (dentry->d_inode) {
		if (data)
			dentry->d_indoe->u.generic_ip = data;
		if (fops)
			dentry->d_indoe->i_fop = fops;
	}
exit:
	return dentry;
}
...
static int __init aufs_init(void)
{
	...
	if (!retval) {
		...
		if (IS_ERR(aufs_mount)) {
			printk (KERN_ERR "aufs: counld not mount!\n");
			unregister_filesystem(&au_fs_type);
			return retval;
		}
	}
	pslot = aufs_create_dir("woman star", NULL);
	aufs_create_file("a", S_IFREG | S_IRUGO, pslot, NULL, NULL);
	aufs_create_file("b", S_IFREG | S_IRUGO, pslot, NULL, NULL);
	aufs_create_file("c", S_IFREG | S_IRUGO, pslot, NULL, NULL);
